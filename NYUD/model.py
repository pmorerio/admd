...
class MultiModal(object):
		...
		with tf.variable_scope('decoder', reuse=reuse):
				with slim.arg_scope([slim.conv2d_transpose], padding='SAME', activation_fn=None,
					...
					with slim.arg_scope([slim.batch_norm], decay=0.95, center=True, scale=True,
						...
						net = slim.conv2d_transpose(net, 512, [3, 3], scope='conv_transpose2')
						net = slim.batch_norm(net, scope='bn2')
						# (batch_size, 56, 56, channels)
						net = slim.conv2d_transpose(net, 256, [5, 5], scope='conv_transpose3')
						net = slim.batch_norm(net, scope='bn3')
						# (batch_size, 112, 112, channels)
						net = slim.conv2d_transpose(net, 128, [5, 5], scope='conv_transpose4')
	...
	def build_model(self):
		...
		if '_rgb' in self.mode or '_depth' in self.mode:
            ...
            loss_distill_depth = tf.nn.softmax_cross_entropy_with_logits(labels=teacher_softmax_depth, logits=self.hall_logits / T)
