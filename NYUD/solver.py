...
class Solver(object):
	...
	for split in splits:
	    ...
	    rgb_classes = sorted(glob.glob(self.image_dir + '/' + split + '/images/*') )
	    depth_classes = sorted(glob.glob(self.image_dir + '/' + split + '/depth/*') )
	    ...
	    self.dataset[split] = {'rgb_images': rgb_images, 'depth_images': depth_images, 'labels': np.squeeze(labels) }
	...
	batches_per_epochs = int(self.dataset[split]['labels'].shape[0] / self.batch_size) + 1 
	#print('Evaluating '+split+' accuracy')
	correct_preds = 0.
	for _im, _lab,  in zip(np.array_split(self.dataset[split][modality+'_images'], batches_per_epochs), 
					np.array_split(self.dataset[split]['labels'], batches_per_epochs),
					):
	    feed_dict = {self.model.images: _im, self.model.labels: _lab, self.model.is_training: split=='train'}
	    _acc_ = session.run(fetches=self.model.accuracy, feed_dict=feed_dict)
	    correct_preds += (_acc_*len(_lab))	# must be a weighted average since last split is smaller				
	print (modality+' '+split+' acc [%.4f]' %(correct_preds/len(self.dataset[split]['labels'])))
	...
	with tf.Session(config=self.config) as sess:
	    ...
	    variables_to_restore = slim.get_model_variables(scope=modality+'/resnet_v1_50')
	    # get rid of logits
	    ...
	    restorer.restore(sess, self.resnet50_ckpt+'resnet_v1_50_'+modality+'.ckpt')
	    print('Loaded!')
	    
	    saver = tf.train.Saver()
	    summary_writer = tf.summary.FileWriter(logdir=self.log_dir, graph=tf.get_default_graph())
	    batches_per_epochs = int(self.dataset['train']['labels'].shape[0] / self.batch_size) + 1 # the +1 gives an additional smaller batch
		...
		if step%10==0:
		    summary, l, acc  = sess.run([self.model.summary_op, self.model.loss, self.model.accuracy], feed_dict)
		...
		if i==0:
		    ...
		    self.eval_all_single_stream('test', session=sess, modality=modality)
		    saver.save(sess, os.path.join(self.model_save_path, modality))
	...
	with tf.Session(config=self.config) as sess:
	    ...
	    variables_to_restore = slim.get_model_variables(scope=modality+'/resnet_v1_50')  
	    restorer = tf.train.Saver(variables_to_restore)
	    restorer.restore(sess, os.path.join(self.model_save_path, modality) )
	    print('Loaded!')
	...
	for rgb_im, depth_im, _lab,  in zip(np.array_split(self.dataset[split]['rgb_images'], batches_per_epochs), 
					    np.array_split(self.dataset[split]['depth_images'], batches_per_epochs),
					    np.array_split(self.dataset[split]['labels'], batches_per_epochs),
					    ):
	    if noise > 0.:
		depth_im = depth_im * np.random.normal(1,noise, size=depth_im.shape)
	    elif noise < 0.:
		depth_im = np.zeros(shape=depth_im.shape)
	    feed_dict = {self.model.rgb_images: rgb_im, self.model.depth_images: depth_im, self.model.labels: _lab, self.model.is_training: split=='train'}
	    _acc_ = session.run(fetches=self.model.accuracy, feed_dict=feed_dict)
	    correct_preds += (_acc_*len(_lab))	# must be a weighted average since last split is smaller				
	print (split+' acc [%.4f]' %(correct_preds/len(self.dataset[split]['labels'])))
	...
	with tf.Session(config=self.config) as sess:
		...
		if i==0:
		    ...
		    saver.save(sess, os.path.join(self.model_save_path, model_name))
	...
	with tf.Session(config=self.config) as sess:
	    ...
	    variables_to_restore = slim.get_model_variables(scope='depth/resnet_v1_50')
	    restorer = tf.train.Saver(variables_to_restore)
	    restorer.restore(sess, os.path.join(self.model_save_path, 'double_stream'))
	    print('Loaded!')
	    
	    # re-initialize the hall strem with values from the depth
	    print ('Copying depth to hallucination...')
	    hall_vars = [var for var in  tf.global_variables() if 'hall' in var.name]
	    depth_vars = [var for var in  tf.global_variables() if 'depth' in var.name]
		...
		#~ if i==0:
		    ...
		    #self.eval_all_double_stream('train', session=sess)
		    self.eval_all_double_stream('test', session=sess)
		    saver.save(sess, os.path.join(self.model_save_path, 'hallucination'))
	...
	with tf.Session(config=self.config) as sess:
	    ...
	    variables_to_restore = slim.get_model_variables(scope='hall/resnet_v1_50')
	    restorer = tf.train.Saver(variables_to_restore)
	    restorer.restore(sess, os.path.join(self.model_save_path, 'hallucination'))
	    print('Loaded!')
	    
	    print ('Loading rgb stream from pretrained double_stream model...')
	    variables_to_restore = slim.get_model_variables(scope='rgb/resnet_v1_50')
	    restorer = tf.train.Saver(variables_to_restore)
	    restorer.restore(sess, os.path.join(self.model_save_path, 'double_stream'))
	    print('Loaded!')
		...
		if i==0:
		    ...
		    #~ self.eval_all_double_stream('train', session=sess)
		    self.eval_all_double_stream('test', session=sess)
		    saver.save(sess, os.path.join(self.model_save_path, 'hallucination_finetuned'))
	...
	with tf.Session(config=self.config) as sess:
	    ...
	    variables_to_restore += slim.get_model_variables(scope='depth/resnet_v1_50')
	    restorer = tf.train.Saver(variables_to_restore)
	    restorer.restore(sess, os.path.join(self.model_save_path, 'double_stream_moddrop'))
	    print('Loaded!')
	...
	with tf.Session(config=self.config) as sess:
		...
		if i==0:
		    saver.save(sess, os.path.join(self.model_save_path, 'autoencoder'))
	...
	with tf.Session(config=self.config) as sess:
	    ...
	    restorer = tf.train.Saver(variables_to_restore)
	    restorer.restore(sess, os.path.join(self.model_save_path, 'autoencoder') )
	    print('Loaded!')

	    summary_writer = tf.summary.FileWriter(logdir=self.log_dir, graph=tf.get_default_graph())
	    batches_per_epochs = int(self.dataset['test']['labels'].shape[0] / self.batch_size) + 1 # the +1 gives an additional smaller batch
	    # simply visualize on the test set
	    # ... and save a copy of the generated images 
	    generated_depth_images = np.zeros((self.n_samples['test'],224,224,3))
		...
		summary, l, gen_depth  = sess.run([self.model.summary_op, self.model.loss, self.model.reconstructed_depth ], feed_dict)
		generated_depth_images[i*self.batch_size:(i+1)*self.batch_size] = gen_depth
		summary_writer.add_summary(summary, step)
		print ('Step: [%d/%d]  loss: [%.6f]' \
	...
	with tf.Session(config=self.config) as sess:
	    ...
	    correct_preds = 0.
	    for rgb_im, depth_im, _lab,  in zip(np.array_split(self.dataset[split]['rgb_images'], batches_per_epochs), 
						np.array_split(generated_depth_images, batches_per_epochs),
						np.array_split(self.dataset[split]['labels'], batches_per_epochs),
						):
		feed_dict = {self.model.rgb_images: rgb_im, self.model.depth_images: depth_im, self.model.labels: _lab, self.model.is_training: split=='train'}
		_acc_ = sess.run(fetches=self.model.accuracy, feed_dict=feed_dict)
		correct_preds += (_acc_*len(_lab))	# must be a weighted average since last split is smaller				
	    print (split+' acc [%.4f]' %(correct_preds/len(self.dataset[split]['labels'])))
