# training one modality (RGB or depth), one stream. with or without bottleneck.
# e.g. python s1_train_stream.py --dset=ntu-mini --modality=depth_bott
...
def train(exp_id, files, args):
    ...
    if args.modality == 'depth' or args.modality == 'rgb':
        bottleneck = False
        modality = args.modality
    elif args.modality == 'depth_bott':
        bottleneck = True
        modality = 'depth'
    ...
    with tf.device('/cpu:0'):
        ...
        dset_train = dset_train.map(
            lambda x: parsers._parse_fun_one_mod(x, is_training=True, modality=modality))
        ...
        if val_filenames:
            ...
            dset_val = dset_val.map(
                lambda x: parsers._parse_fun_one_mod(x, is_training=False, modality=modality))
    ...
    with tf.device(args.gpu0):
        with slim.arg_scope(resnet_v1.resnet_arg_scope(batch_norm_decay=args.bn_decay)):
            ...
            net_out, net_endpoints = resnet_v1.resnet_one_stream_main(
                batch_images, nr_frames, num_classes=n_classes, scope=scope, gpu_id=args.gpu0, is_training=is_training, bottleneck=bottleneck)
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        if args.just_eval:
            restorers.restore_weights_s1_continue(sess, args.ckpt, modality)
        else:
