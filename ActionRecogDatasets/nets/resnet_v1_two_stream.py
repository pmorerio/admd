        ...
        with tf.variable_scope(scope, 'resnet_v1', [inputs], reuse=reuse) as sc:
            ...
            with slim.arg_scope([slim.conv2d, bottleneck,
                ...
                with slim.arg_scope([slim.batch_norm], is_training=is_training):
                    ...
                    if bottleneck:
                        net = slim.conv2d(net, 128, 1, scope='conv_bott')
                    last_pool = net
...
def resnet_one_stream_main(inputs, nr_frames,
                           num_classes=None,
                           scope='resnet_v1_50', is_training=True, gpu_id='/gpu:0', bottleneck=True):
    blocks = resnet_v1_50(bottleneck_normal)
    net, endpoints = resnet_one_stream(inputs, blocks, nr_frames,
                                       num_classes=num_classes,
                                       scope=scope, is_training=is_training, gpu_id=gpu_id, bottleneck=bottleneck)
...
def feature_discriminator(inputs, temporal_order, n_classes=60, reuse=False, is_training=True):
    inputs = tf.concat(
        axis=-1, values=[tf.cast(temporal_order, tf.float32), inputs])
    ...
    with tf.variable_scope('disc_e', reuse=reuse):
        with slim.arg_scope([slim.fully_connected], weights_initializer=tf.contrib.layers.xavier_initializer(), biases_initializer=tf.zeros_initializer()):
            with slim.arg_scope([slim.batch_norm], decay=0.95, center=True, scale=True,
                                activation_fn=lrelu, is_training=is_training):
                net = slim.conv2d(
                    inputs, 256, 1, activation_fn=lrelu, scope='sdisc_fc1')
                net = slim.conv2d(
                    net, 128, 1, activation_fn=lrelu, scope='sdisc_fc3')
                net = slim.conv2d(
                    net, n_classes + 1, 1, activation_fn=None, scope='sdisc_prob')
                return net
